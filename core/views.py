...
def tao_du_lieu_mau(request):
    ...
    # 2. DANH SÁCH 15 TRẠM XĂNG KHẮP TP.HCM
    danh_sach_tram = [
    ...
    # Vòng lặp tạo 15 trạm và tự động sinh bồn chứa
    for index, data in enumerate(danh_sach_tram):
        ...
        # Tạo mức nhiên liệu ngẫu nhiên để test cảnh báo
        # Trạm số 1 (Trung tâm) cố định để dễ test Admin Import
        if index == 0:
            BonChua.objects.create(tram=tram, ten_bon="Bồn A95", loai_nhien_lieu='A95', suc_chua_toi_da=15000, muc_hien_tai=12000)
            BonChua.objects.create(tram=tram, ten_bon="Bồn E5", loai_nhien_lieu='E5', suc_chua_toi_da=10000, muc_hien_tai=500) # Sắp hết
    ...
    # 3. TẠO 4 NHÀ CUNG CẤP (Kho Xăng Dầu)
    ...
    # 4. TẠO DỮ LIỆU TIN TỨC & SẢN PHẨM (Cho trang chủ)
